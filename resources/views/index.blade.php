        ...
        <div class="wrapper d-flex flex-column">
            @foreach ($movies as $movie)
				<div class="special card d-lg-block border-0 border-bottom">
					<img src="{{ $movie->image }}" alt="A woman sitting on a rock outside in the evening"
						class="float-start">
					<span class="type fs-6 fw-light text-uppercase mt-3">{{ $movie->type }}</span>
					<h3 class="fs-5">{{ $movie->title }}</h3>
					<p class="fs-6 mb-3">{{ $movie->description }}</p>
					<div class="wrapper-time d-flex flex-wrap align-items-center justify-content-start">
						<span class="view fs-6 text-uppercase me-3"><i class="far fa-clock"></i>Viewing times:
							{{ $movie->day }}</span>
